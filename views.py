...
def index(request):
    if request.method == "POST":
        ...
        if myform.is_valid():
            ...
            age = myform.cleaned_data['age_value']
            sex = myform.cleaned_data['sex_value']
            ca = myform.cleaned_data['ca_value']
            # Uncomment and use the target below to test app functions
            target = 1 + cp + thalach + exang + oldpeak + slope + thal + age + sex + ca
            m_inputs = [[cp, thalach, exang, oldpeak, slope, thal, age, sex, ca]]
